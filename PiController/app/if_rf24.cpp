...
const uint16_t ac_node_address =		01;
		...
		while (msg_available(GW_TASK_IF_RF24_ID)) {
			...
			case GW_RF24_IF_PURE_MSG_OUT: {
				...
				if (rf_pending_flag == APP_FLAG_OFF) {
					RF24NetworkHeader send_header(ac_node_address, RF24_DATA_PURE_MSG_TYPE);
			...
			case GW_RF24_IF_COMMON_MSG_OUT: {
				...
				if (rf_pending_flag == APP_FLAG_OFF) {
					RF24NetworkHeader send_header(ac_node_address, RF24_DATA_COMMON_MSG_TYPE);
			...
			case GW_RF24_IF_TIMER_PACKET_DELAY: {
				...
				if (!fifo_is_empty(&send_pure_fifo)) {
					...
					RF24NetworkHeader ac_header(ac_node_address, RF24_DATA_PURE_MSG_TYPE);
					...
					RF24NetworkHeader ac_header(ac_node_address, RF24_DATA_COMMON_MSG_TYPE);
