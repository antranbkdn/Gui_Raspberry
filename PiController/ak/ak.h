...
signals: void progress_changed(uint32_t ak_thread_mt_started);  //edit code
public slots:
     void sendData();
     void transmit();
private:// edited by Supper Man
    int runmain();
  //  connect(const QObject *sender, const char *signal, const char *member, Qt::ConnectionType type) const
};
